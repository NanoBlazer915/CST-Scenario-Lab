...
resource "aws_instance" "mtu"{
  ...
  user_data = templatefile("${path.module}/scripts/mtu.sh.tpl", {
  ...
  tags = {
    Name        = "client-mtu-${var.name_prefix}-${var.random_pet_id}"
    "AutoDestroy" = "true"
...
resource "aws_network_interface" "clientmtu_private_nic1" {
#  count            = 1
...
resource "aws_network_interface_attachment" "clientmtu_nic1_attachment" {
#  count                = 1
  instance_id          = aws_instance.mtu.id
  network_interface_id = aws_network_interface.clientmtu_private_nic1.id
  device_index         = 1
}
