...
resource "aws_instance" "memory"{
  ...
  user_data = templatefile("${path.module}/scripts/memory.sh.tpl", {
  ...
  tags = {
    Name        = "client-memory-${var.name_prefix}-${var.random_pet_id}"
    "AutoDestroy" = "true"
...
resource "aws_network_interface" "clientmemory_private_nic1" {
#  count            = 1
...
resource "aws_network_interface_attachment" "clientmemory_nic1_attachment" {
#  count                = 1
  instance_id          = aws_instance.memory.id
  network_interface_id = aws_network_interface.clientmemory_private_nic1.id
  device_index         = 1
}
