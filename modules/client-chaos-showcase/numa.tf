...
resource "aws_instance" "numa"{
  ...
  user_data = templatefile("${path.module}/scripts/numa.sh.tpl", {
 ...
 # Define the root block device
  ...
  tags = {
    Name        = "client-numa-${var.name_prefix}-${var.random_pet_id}"
    "AutoDestroy" = "true"
...
resource "aws_network_interface" "clientnuma_private_nic1" {
#  count            = 1
...
resource "aws_network_interface_attachment" "clientnuma_nic1_attachment" {
#  count                = 1
  instance_id          = aws_instance.numa.id
  network_interface_id = aws_network_interface.clientnuma_private_nic1.id
  device_index         = 1
}
